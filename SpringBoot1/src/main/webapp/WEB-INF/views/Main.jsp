...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
		...
		<body>
					...
					<div class="inner">
						...
						<ul class="actions vertical">
							...
							<form action="join" method="post">
								...
								<li><span id="resultEamil"> </span></li>
								<li><input type="password" name="pw" placeholder="PW를 입력하세요"></li>
			...
			<script src="/web/assets/js/main.js"></script>
 			...
 			<!--  비동기 통신으로 회원가입의 이메일 중복 여부를 체크할 수 있는 기능 -->
 			<script type="text/javascript">
 				function checkEmail(){
 					...
 					// 1) 요청 url
 					// 2) 요청 type (get/post)
 					// 3) 활용 data -> J?SON타입 / key:value
 					// 4) 성공/실패에 대한 로직 
 					$.ajax({
 						url:"checkEmail", // controller가 받을 수 있는 요청값
 						type: "get",
 						data:{inputEmail:email},
 						success: function(res){
 							// 매개변수의 res의 의미 : controller의 작업 이후
 							// 중복체크 여부에 대한 결과값이 담기는 변수!
 							// res -> restcontroller에서 전달해주는 return값 "true"/"false"
 							if(res == "true"){
 								$("#resultEamil").text("사용 가능한 이메일 입니다").css("color","green")
 							}else{
 								$("#resultEamil").text("이미 사용중인 이메일입니다").css("color","red")
 							} 
 						}
 						error : function(){
 							alert ("회원 체크 실패");
