...
public class MyController {
	...
	// 업데이트 기능을 위한 요청 진행하기
	// 1. 요청mapping
	// 2. 응답의 구조 생성 (from태그를 통해 입력받은 데이터를 활용)
	// 3. DB에 해당 데이터를 전달 -> update spl구문 실행
	// --> email이 일치할 때 pw,tel,address가 수정 되도록
	// 4. 결과 화면은 main으로  
	@PostMapping("/update")
	...
	//관리자의 입장에서 지금까지 회원가입된 정보를 알아보기 위해 시행 
	@GetMapping("/showMember")
	public String showMember(Model model) {
		
		List<MsgMember> list = mapper.showMember();
		model.addAttribute("list",list);
		// 몇개의 정보를 가질지 몰라서 리스트로 받아준다 
	...
	public String delete(@RequestParam("email")String email) {
		...
		// 쿼리 파라미터중 email 값을 String email 변수 바인딩
		mapper.delete(email);
